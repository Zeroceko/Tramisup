...
export default function ActionsSection({
  ...
  const priorityLabel: Record<string, string> = isEn
  ...
  return (
    <div className="bg-white rounded-[15px] border border-[#e8e8e8] p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#666d80] mb-0.5">
            {isEn ? "Tasks" : "Görevler"}
          </p>
          <h2 className="text-[16px] font-semibold text-[#0d0d12]">
            {isEn ? "Action list" : "Aksiyon Listesi"}
          </h2>
        </div>
        <button
      ...
      {showAddForm && (
        <form onSubmit={addTask} className="mb-4 p-4 bg-[#f6f6f6] rounded-[12px] space-y-3">
          ...
          >
            {loading === "new" ? (isEn ? "Adding…" : "Ekleniyor…") : isEn ? "Add task" : "Görev Ekle"}
          </button>
        </form>
      )}
      ...
      <div className="space-y-2">
        {pendingTasks.length === 0 && !showAddForm && (
          <p className="text-center text-[13px] text-[#9ca3af] py-8">
            {isEn ? "No pending tasks" : "Bekleyen görev yok"}
          </p>
        )}
        ...
        {completedTasks.length > 0 && (
          <div className="pt-3 border-t border-[#e8e8e8]">
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#9ca3af] mb-2">
              {isEn ? `Completed (${completedTasks.length})` : `Tamamlandı (${completedTasks.length})`}
            </p>
