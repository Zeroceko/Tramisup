  ...
  return (
    <div className="bg-white rounded-[15px] border border-[#e8e8e8] p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#666d80] mb-0.5">{isEn ? "Habits" : "Alışkanlıklar"}</p>
          <h2 className="text-[16px] font-semibold text-[#0d0d12]">{isEn ? "Growth routines" : "Growth Rutinler"}</h2>
        </div>
        <button
        ...
        >
          {showAddForm ? (isEn ? "Cancel" : "İptal") : isEn ? "+ Add" : "+ Ekle"}
        </button>
      </div>
      ...
      {showAddForm && (
        <form onSubmit={addRoutine} className="mb-4 p-4 bg-[#f6f6f6] rounded-[12px] space-y-3">
          <input
            type="text"
            placeholder={isEn ? "Routine title" : "Rutin başlığı"}
          ...
          />
          <textarea
            placeholder={isEn ? "Description (optional)" : "Açıklama (opsiyonel)"}
          ...
          >
            ...
            <option value="MONTHLY">{isEn ? "Monthly" : "Aylık"}</option>
          </select>
          <button
            type="submit"
          ...
          >
            {loading === "new" ? (isEn ? "Adding…" : "Ekleniyor…") : isEn ? "Add routine" : "Rutin Ekle"}
          </button>
        </form>
      )}
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {routine.lastCompletedAt && (
                    <span className="text-[11px] text-[#9ca3af]">
                      {isEn ? "Last:" : "Son:"}{" "}
                      {new Date(routine.lastCompletedAt).toLocaleDateString(isEn ? "en-US" : "tr-TR")}
                    </span>
                  )}
                </div>
              </div>
